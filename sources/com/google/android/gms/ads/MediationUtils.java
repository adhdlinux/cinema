    ...
    public static AdSize findClosestSize(Context context, AdSize adSize, List<AdSize> list) {
        ...
        if (!(list == null || adSize == null)) {
            if (!adSize.zzh() && !adSize.zzi()) {
                ...
                adSize = new AdSize(Math.round(((float) adSize.getWidthInPixels(context)) / f2), Math.round(((float) adSize.getHeightInPixels(context)) / f2));
            }
            for (AdSize next : list) {
                if (next != null) {
                    ...
                    if (((double) width) * 0.5d <= ((double) width2) && width >= width2) {
                        if (adSize.zzi()) {
                            int zza = adSize.zza();
                            if (((Integer) zzba.zzc().zzb(zzbbm.zzhu)).intValue() <= width2) {
                                if (((Integer) zzba.zzc().zzb(zzbbm.zzhv)).intValue() <= height2) {
                                    if (zza < height2) {
                                    }
                                }
                            }
