...
public final class zzakq {
                ...
                if (zza >= longValue) {
                    ...
                    if (allocate.getLong(8) == 2334950737559900225L && allocate.getLong(16) == 3617552046287187010L) {
                        ...
                        if (j3 < ((long) allocate.capacity()) || j3 > 2147483639) {
                            throw new zzakn("APK Signing Block size out of range: " + j3);
                        }
                        int i3 = (int) (8 + j3);
                        long j4 = zza - ((long) i3);
                        if (j4 >= 0) {
                                            ...
                                            while (slice.hasRemaining()) {
                                                ...
                                                if (slice.remaining() >= 8) {
                                                    ...
                                                    if (j6 < 4 || j6 > 2147483647L) {
                                                        throw new zzakn("APK Signing Block entry #" + i2 + " size out of range: " + j6);
                                                    }
                                                    int i4 = (int) j6;
                                                    ...
                                                    if (i4 > slice.remaining()) {
                                                        ...
                                                        X509Certificate[][] zzl = zzl(randomAccessFile.getChannel(), new zzakm(zze(slice, i4 - 4), longValue2, zza, longValue, byteBuffer2, (zzakl) null));
                            ...
                            throw new zzakn("APK Signing Block sizes in header and footer do not match: " + j5 + " vs " + j3);
                        }
                        throw new zzakn("APK Signing Block offset out of range: " + j4);
                    }
    ...
    private static ByteBuffer zzf(ByteBuffer byteBuffer) throws IOException {
        if (byteBuffer.remaining() >= 4) {
            ...
            if (i2 < 0) {
                ...
                throw new IOException("Length-prefixed field longer than remaining buffer. Field length: " + i2 + ", remaining: " + remaining);
            }
        } else {
    ...
    private static void zzg(int i2, byte[] bArr, int i3) {
        bArr[1] = (byte) (i2 & JfifUtil.MARKER_FIRST_BYTE);
        bArr[2] = (byte) ((i2 >>> 8) & JfifUtil.MARKER_FIRST_BYTE);
        bArr[3] = (byte) ((i2 >>> 16) & JfifUtil.MARKER_FIRST_BYTE);
        bArr[4] = (byte) (i2 >> 24);
    }
    ...
    private static void zzh(Map map, FileChannel fileChannel, long j2, long j3, long j4, ByteBuffer byteBuffer) throws SecurityException {
        if (!map.isEmpty()) {
            zzakk zzakk = new zzakk(fileChannel, 0, j2);
            zzakk zzakk2 = new zzakk(fileChannel, j3, j4 - j3);
            ByteBuffer duplicate = byteBuffer.duplicate();
            duplicate.order(ByteOrder.LITTLE_ENDIAN);
            long j5 = j2;
            zzakr.zzd(duplicate, j2);
            zzaki zzaki = new zzaki(duplicate);
            int size = map.size();
            ...
            try {
                byte[][] zzk = zzk(iArr, new zzakj[]{zzakk, zzakk2, zzaki});
                while (i2 < size) {
                    int i4 = iArr[i2];
                    Map map2 = map;
                    if (MessageDigest.isEqual((byte[]) map.get(Integer.valueOf(i4)), zzk[i2])) {
                        i2++;
                    } else {
    ...
    private static byte[] zzi(ByteBuffer byteBuffer) throws IOException {
        ...
        if (i2 < 0) {
            ...
            int remaining = byteBuffer.remaining();
            throw new IOException("Underflow while reading length-prefixed value. Length: " + i2 + ", available: " + remaining);
        }
    }
                    ...
                    if (arrayList.equals(arrayList2)) {
                        ...
                        if (bArr3 == null || MessageDigest.isEqual(bArr3, bArr)) {
                            ...
                            while (zzf6.hasRemaining()) {
                                ...
                                byte[] zzi2 = zzi(zzf6);
                                try {
                                    arrayList3.add(new zzako((X509Certificate) certificateFactory.generateCertificate(new ByteArrayInputStream(zzi2)), zzi2));
    ...
    private static byte[][] zzk(int[] iArr, zzakj[] zzakjArr) throws DigestException {
        long j2;
        int i2;
        int length;
        int[] iArr2 = iArr;
        ...
        while (true) {
            ...
            j4 += (zzakjArr[i4].zza() + 1048575) / PlaybackStateCompat.ACTION_SET_CAPTIONING_ENABLED;
            i4++;
        }
        if (j4 < 2097151) {
            ...
            for (i2 = 3; i8 < i2; i2 = 3) {
                ...
                while (zza > j3) {
                    ...
                    long j6 = j5;
                    try {
                        zzakj.zzb(messageDigestArr, j6, min);
                        byte[] bArr4 = bArr3;
                        int i11 = 0;
            ...
            while (i16 < iArr2.length) {
                ...
                String zzd2 = zzd(i17);
                try {
                    bArr7[i16] = MessageDigest.getInstance(zzd2).digest(bArr8);
                    i16++;
        ...
        throw new DigestException("Too many chunks: " + j4);
    }

    private static X509Certificate[][] zzl(FileChannel fileChannel, zzakm zzakm) throws SecurityException {
        HashMap hashMap = new HashMap();
        ArrayList arrayList = new ArrayList();
        try {
            CertificateFactory instance = CertificateFactory.getInstance("X.509");
            try {
                ByteBuffer zzf = zzf(zzakm.zza);
                int i2 = 0;
                ...
                if (i2 <= 0) {
                    ...
                    zzh(hashMap, fileChannel, zzakm.zzb, zzakm.zzc, zzakm.zzd, zzakm.zze);
