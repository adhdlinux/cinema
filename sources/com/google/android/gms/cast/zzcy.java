package com.google.android.gms.cast;

import com.google.android.gms.cast.internal.zzan;
import java.util.List;

final class zzcy implements zzan {
    final /* synthetic */ RemoteMediaPlayer zza;

    zzcy(RemoteMediaPlayer remoteMediaPlayer) {
        this.zza = remoteMediaPlayer;
    }

    public final void zza() {
    }

    public final void zzb(MediaError mediaError) {
    }

    public final void zzc() {
        RemoteMediaPlayer.zze(this.zza);
    }

    public final void zzd() {
        RemoteMediaPlayer.zzf(this.zza);
    }

    public final void zze(int[] iArr) {
    }

    public final void zzf(int[] iArr, int i2) {
    }

    public final void zzg(MediaQueueItem[] mediaQueueItemArr) {
    }

    public final void zzh(int[] iArr) {
    }

    public final void zzi(List list, List list2, int i2) {
    }

    public final void zzj(int[] iArr) {
    }

    public final void zzk() {
        RemoteMediaPlayer.zzg(this.zza);
    }

    public final void zzl() {
    }

    public final void zzm() {
        RemoteMediaPlayer.zzh(this.zza);
    }
}
