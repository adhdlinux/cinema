...
class CacheThreadHandler extends Handler {
    ...
    private WebRequest getWebRequest(String str, int i2, int i3, HashMap<String, List<String>> hashMap) throws MalformedURLException {
        HashMap hashMap2 = new HashMap();
        if (hashMap != null) {
            hashMap2.putAll(hashMap);
        }
        return new WebRequest(str, "GET", hashMap2, i2, i3);
    }

    private void postProcessDownload(long j2, String str, File file, long j3, long j4, boolean z2, int i2, Map<String, List<String>> map, CacheEventSender cacheEventSender) {
        String str2 = str;
        ...
        if (!z2) {
            DeviceLog.debug("Unity Ads cache: File " + file.getName() + " of " + j3 + " bytes downloaded in " + elapsedRealtime + "ms");
            cacheEventSender2.sendEvent(CacheEvent.DOWNLOAD_END, str2, Long.valueOf(j3), Long.valueOf(j4), Long.valueOf(elapsedRealtime), Integer.valueOf(i2), Request.getResponseHeadersMap(map));
            return;
        }
        long j5 = j3;
        DeviceLog.debug("Unity Ads cache: downloading of " + str2 + " stopped");
        cacheEventSender2.sendEvent(CacheEvent.DOWNLOAD_STOPPED, str2, Long.valueOf(j3), Long.valueOf(j4), Long.valueOf(elapsedRealtime), Integer.valueOf(i2), Request.getResponseHeadersMap(map));
    }
    ...
    public void handleMessage(Message message) {
        ...
        data.remove("source");
        String string2 = data.getString(TouchesHelper.TARGET_KEY);
        data.remove(TouchesHelper.TARGET_KEY);
        ...
        if ((z2 && !file.exists()) || (!z2 && file.exists())) {
            ...
            downloadFile(string, string2, i2, i3, i4, hashMap, z2, cacheEventSender);
        }
    }
