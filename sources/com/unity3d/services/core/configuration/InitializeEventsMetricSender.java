...
public class InitializeEventsMetricSender implements IInitializeEventsMetricSender, IInitializationListener {
    ...
    private Metric getPrivacyRequestMetric(boolean z2) {
        if (z2) {
    ...
    private void sendConfigResolutionRequestIfNeeded(boolean z2) {
    ...
    private void sendPrivacyResolutionRequestIfNeeded(boolean z2) {
        if (this._privacyConfigStartTime == 0 || this._privacyConfigEndTime == 0) {
            ...
            sendMetric(getPrivacyRequestMetric(z2));
        }
    }

    private void sendTokenAvailabilityMetricWithConfig(boolean z2) {
        Metric metric;
    ...
    public void didConfigRequestEnd(boolean z2) {
        this._configEndTime = System.nanoTime();
        sendConfigResolutionRequestIfNeeded(z2);
    }
    ...
    public void didPrivacyConfigRequestEnd(boolean z2) {
        this._privacyConfigEndTime = System.nanoTime();
        sendPrivacyResolutionRequestIfNeeded(z2);
    }
    ...
    public synchronized void sdkTokenDidBecomeAvailableWithConfig(boolean z2) {
        if (!this._tokenMetricSent) {
            sendTokenAvailabilityMetricWithConfig(z2);
            if (z2) {
