...
public class DeviceInfoReaderBuilder {
    ...
    private JsonFlattenerRules getTsiRequestStorageRules() {
        return new JsonFlattenerRules(Arrays.asList(new String[]{"privacy", GDPR.GDPR_STANDARD, "framework", "adapter", "mediation", "unity", "pipl", "configuration", "user", JsonStorageKeyNames.UNIFIED_CONFIG_KEY}), Collections.singletonList(AppMeasurementSdk.ConditionalUserProperty.VALUE), Arrays.asList(new String[]{HlsSegmentFormat.TS, "exclude", "pii", "nonBehavioral", "nonbehavioral"}));
    }
