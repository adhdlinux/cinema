...
public class HeadlessJsTaskContext {
    ...
    private void scheduleTaskTimeout(final int i2, long j2) {
        ...
        this.mTaskTimeouts.append(i2, r02);
        this.mHandler.postDelayed(r02, j2);
    }
    ...
    public synchronized void finishTask(final int i2) {
        boolean z2;
        boolean remove = this.mActiveTasks.remove(Integer.valueOf(i2));
        Assertions.assertCondition(remove, "Tried to finish non-existent task with id " + i2 + ".");
        if (this.mActiveTaskConfigs.remove(Integer.valueOf(i2)) != null) {
            z2 = true;
        } else {
            z2 = false;
        }
        Assertions.assertCondition(z2, "Tried to remove non-existent task config with id " + i2 + ".");
        removeTimeout(i2);
        UiThreadUtil.runOnUiThread(new Runnable() {
            public void run() {
                for (HeadlessJsTaskEventListener onHeadlessJsTaskFinish : HeadlessJsTaskContext.this.mHeadlessJsTaskEventListeners) {
                    onHeadlessJsTaskFinish.onHeadlessJsTaskFinish(i2);
                }
            }
        });
    }
    ...
    public synchronized boolean retryTask(final int i2) {
        ...
        Assertions.assertCondition(z2, "Tried to retrieve non-existent task config with id " + i2 + ".");
        ...
        UiThreadUtil.runOnUiThread(new Runnable() {
            public void run() {
                HeadlessJsTaskContext.this.startTask(headlessJsTaskConfig2, i2);
            }
    ...
    public synchronized void startTask(HeadlessJsTaskConfig headlessJsTaskConfig, int i2) {
        ...
        for (HeadlessJsTaskEventListener onHeadlessJsTaskStart : this.mHeadlessJsTaskEventListeners) {
            onHeadlessJsTaskStart.onHeadlessJsTaskStart(i2);
        }
    }
}
