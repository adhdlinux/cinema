...
public class UIManagerHelper {
    ...
    public static float[] getDefaultTextInputPadding(ThemedReactContext themedReactContext) {
        ...
        return new float[]{PixelUtil.toDIPFromPixel((float) ViewCompat.I(editText)), PixelUtil.toDIPFromPixel((float) ViewCompat.H(editText)), PixelUtil.toDIPFromPixel((float) editText.getPaddingTop()), PixelUtil.toDIPFromPixel((float) editText.getPaddingBottom())};
    }
    ...
    public static EventDispatcher getEventDispatcher(ReactContext reactContext, int i2) {
        ...
        if (uIManager == null) {
            String str = TAG;
            ReactSoftExceptionLogger.logSoftException(str, new ReactNoCrashSoftException("Unable to find UIManager for UIManagerType " + i2));
            return null;
        }
        EventDispatcher eventDispatcher = (EventDispatcher) uIManager.getEventDispatcher();
        if (eventDispatcher == null) {
            String str2 = TAG;
            ReactSoftExceptionLogger.logSoftException(str2, new IllegalStateException("Cannot get EventDispatcher for UIManagerType " + i2));
        }
    ...
    public static EventDispatcher getEventDispatcherForReactTag(ReactContext reactContext, int i2) {
        ...
        if (eventDispatcher == null) {
            String str = TAG;
            ReactSoftExceptionLogger.logSoftException(str, new IllegalStateException("Cannot get EventDispatcher for reactTag " + i2));
        }
        return eventDispatcher;
    }
    ...
    private static UIManager getUIManager(ReactContext reactContext, int i2, boolean z2) {
        if (reactContext.isBridgeless()) {
            ...
            try {
                ...
                String str = TAG;
                ReactSoftExceptionLogger.logSoftException(str, new ReactNoCrashSoftException("Cannot get UIManager for UIManagerType: " + i2));
                return (UIManager) catalystInstance.getNativeModule(cls);
            }
        }
    }
