...
public class NumberFormat {
    ...
    private void initializeNumberFormat(List<String> list, Map<String, Object> map) throws JSRangeErrorException {
        ...
        if (JSObjects.isUndefined(GetOption) || isLocaleIdType(JSObjects.getJavaString(GetOption))) {
            ...
            if (this.mResolvedStyle == IPlatformNumberFormatter.Style.CURRENCY) {
                if (Build.VERSION.SDK_INT >= 24) {
                    i2 = PlatformNumberFormatterICU.getCurrencyDigits(this.mResolvedCurrency);
                } else {
                    i2 = PlatformNumberFormatterAndroid.getCurrencyDigits(this.mResolvedCurrency);
                }
                double d2 = (double) i2;
                obj = JSObjects.newNumber(d2);
                obj2 = JSObjects.newNumber(d2);
            } else {
    ...
    public List<Map<String, String>> formatToParts(double d2) throws JSRangeErrorException {
        ...
        AttributedCharacterIterator formatToParts = this.mPlatformNumberFormatter.formatToParts(d2);
        ...
        for (char first = formatToParts.first(); first != 65535; first = formatToParts.next()) {
            ...
            if (formatToParts.getIndex() + 1 == formatToParts.getRunLimit()) {
                ...
                if (it2.hasNext()) {
                    str = this.mPlatformNumberFormatter.fieldToString(it2.next(), d2);
                } else {
                    str = "literal";
                }
