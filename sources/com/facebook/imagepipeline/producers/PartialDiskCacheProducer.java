...
public class PartialDiskCacheProducer implements Producer<EncodedImage> {
    ...
    private static class PartialDiskCacheConsumer extends DelegatingConsumer<EncodedImage, EncodedImage> {
        ...
        private void copy(InputStream inputStream, OutputStream outputStream, int i2) throws IOException {
            ...
            this.mByteArrayPool.release(bArr);
            if (i3 > 0) {
                throw new IOException(String.format((Locale) null, "Failed to read %d bytes - finished %d short", new Object[]{Integer.valueOf(i2), Integer.valueOf(i3)}));
            }
        }
        ...
        private PooledByteBufferOutputStream merge(EncodedImage encodedImage, EncodedImage encodedImage2) throws IOException {
            ...
            PooledByteBufferOutputStream newOutputStream = this.mPooledByteBufferFactory.newOutputStream(encodedImage2.getSize() + i2);
            copy(encodedImage.getInputStreamOrThrow(), newOutputStream, i2);
        ...
        public void onNewResultImpl(EncodedImage encodedImage, int i2) {
            if (!BaseConsumer.isNotLast(i2)) {
                ...
                } else if (!BaseConsumer.statusHasFlag(i2, 8) || !BaseConsumer.isLast(i2) || encodedImage == null || encodedImage.getImageFormat() == ImageFormat.UNKNOWN) {
        ...
        return new Continuation<EncodedImage, Void>() {
            public Void then(Task<EncodedImage> task) throws Exception {
                if (PartialDiskCacheProducer.isTaskCancelled(task)) {
                    ...
                    producerListener.onProducerFinishWithFailure(producerContext2, PartialDiskCacheProducer.PRODUCER_NAME, task.m(), (Map<String, String>) null);
                    ...
                    if (n2 != null) {
                        ...
                        producerListener2.onProducerFinishWithSuccess(producerContext, PartialDiskCacheProducer.PRODUCER_NAME, PartialDiskCacheProducer.getExtraMap(producerListener2, producerContext, true, n2.getSize()));
                        BytesRange max = BytesRange.toMax(n2.getSize() - 1);
                        n2.setBytesRange(max);
                        int size = n2.getSize();
                        ...
                        if (max.contains(imageRequest.getBytesRange())) {
                            ...
                            consumer2.onNewResult(n2, 9);
                        } else {
                            consumer2.onNewResult(n2, 8);
                            PartialDiskCacheProducer.this.startInputProducer(consumer2, new SettableProducerContext(ImageRequestBuilder.fromRequest(imageRequest).setBytesRange(BytesRange.from(size - 1)).build(), producerContext2), cacheKey2, n2);
                        }
                    } else {
                        ...
                        PartialDiskCacheProducer.this.startInputProducer(consumer2, producerContext2, cacheKey2, n2);
                    }
                }
                return null;
            }
