...
public class PriorityNetworkFetcher<FETCH_STATE extends FetchState> implements NetworkFetcher<PriorityFetchState<FETCH_STATE>> {
    ...
    public static class PriorityFetchState<FETCH_STATE extends FetchState> extends FetchState {
        ...
        private PriorityFetchState(Consumer<EncodedImage> consumer, ProducerContext producerContext, FETCH_STATE fetch_state, long j2, int i2, int i3, int i4) {
            ...
            this.isInitialPriorityHigh = producerContext.getPriority() == Priority.HIGH ? true : z2;
            this.currentlyFetchingCountWhenCreated = i4;
        }
    }
    ...
    /* access modifiers changed from: private */
    public void changePriority(PriorityFetchState<FETCH_STATE> priorityFetchState, boolean z2) {
        ...
        synchronized (this.mLock) {
            ...
            FLog.v(str2, "change-pri: %s %s", (Object) str, (Object) priorityFetchState.getUri());
            priorityFetchState.priorityChangedCount++;
            putInQueue(priorityFetchState, z2);
        ...
        try {
            this.mDelegate.fetch(priorityFetchState.delegatedState, new NetworkFetcher.Callback() {
                ...
                public void onFailure(Throwable th) {
                    boolean z2;
                    if (PriorityNetworkFetcher.this.maxNumberOfRequeue == -1 || priorityFetchState.requeueCount < PriorityNetworkFetcher.this.maxNumberOfRequeue) {
                        z2 = true;
                    } else {
                        z2 = false;
                    }
                    if (!z2 || (th instanceof NonrecoverableException)) {
    ...
    private void moveDelayedRequestsToPriorityQueues() {
        ...
        if (!this.mDelayedQueue.isEmpty() && this.mClock.now() - this.firstDelayedRequestEnqueuedTimeStamp > this.requeueDelayTimeInMillis) {
            Iterator<PriorityFetchState<FETCH_STATE>> it2 = this.mDelayedQueue.iterator();
            while (it2.hasNext()) {
                PriorityFetchState next = it2.next();
                if (next.getContext().getPriority() == Priority.HIGH) {
                    z2 = true;
                } else {
                    z2 = false;
                }
    ...
    private void putInQueue(PriorityFetchState<FETCH_STATE> priorityFetchState, boolean z2) {
        if (!z2) {
    ...
    public void requeue(PriorityFetchState<FETCH_STATE> priorityFetchState) {
        synchronized (this.mLock) {
            ...
            int i2 = this.immediateRequeueCount;
            if (i2 == -1 || priorityFetchState.requeueCount <= i2) {
    ...
    public void fetch(final PriorityFetchState<FETCH_STATE> priorityFetchState, final NetworkFetcher.Callback callback) {
        ...
        synchronized (this.mLock) {
            ...
            FLog.v(TAG, "enqueue: %s %s", (Object) z2 ? "HI-PRI" : "LOW-PRI", (Object) priorityFetchState.getUri());
    ...
    public PriorityNetworkFetcher(NetworkFetcher<FETCH_STATE> networkFetcher, boolean z2, int i2, int i3, boolean z3, int i4, boolean z4, int i5, int i6, boolean z5, MonotonicClock monotonicClock) {
        ...
        this.mIsHiPriFifo = z2;
        this.mMaxOutstandingHiPri = i2;
        this.mMaxOutstandingLowPri = i3;
        if (i2 > i3) {
            this.inflightFetchesCanBeCancelled = z3;
            this.maxNumberOfRequeue = i4;
            this.doNotCancelRequests = z4;
            this.immediateRequeueCount = i5;
            this.requeueDelayTimeInMillis = (long) i6;
            this.multipleDequeue = z5;
