...
public class RequestLoggingListener implements RequestListener {
    ...
    public synchronized void onRequestStart(ImageRequest imageRequest, Object obj, String str, boolean z2) {
        if (FLog.isLoggable(2)) {
            FLog.v(TAG, "time %d: onRequestSubmit: {requestId: %s, callerContext: %s, isPrefetch: %b}", (Object) Long.valueOf(getTime()), (Object) str, obj, (Object) Boolean.valueOf(z2));
    ...
    public synchronized void onUltimateProducerReached(String str, String str2, boolean z2) {
        if (FLog.isLoggable(2)) {
            Pair create = Pair.create(str, str2);
            long time = getTime();
            FLog.v(TAG, "time %d: onUltimateProducerReached: {requestId: %s, producer: %s, elapsedTime: %d ms, success: %b}", Long.valueOf(time), str, str2, Long.valueOf(getElapsedTime(this.mProducerStartTimeMap.remove(create), time)), Boolean.valueOf(z2));
        }
    }
