...
public class GamoVideo extends PremiumResolver {
    public String c() {
        return "GamoVideo";
    }
    ...
    public void l(MediaSource mediaSource, ObservableEmitter<? super MediaSource> observableEmitter) {
        String a2 = Regex.a(mediaSource.getStreamLink(), "(?://|\\.)(gamovideo\\.com)/(?:embed-)([0-9a-zA-Z]+)", 2);
        if (!a2.isEmpty()) {
            super.l(mediaSource, observableEmitter);
            if (!Utils.f37611d) {
                String str = "https://gamovideo.com" + a2 + "/";
